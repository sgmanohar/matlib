...
solution=struct();
  ...
  switch fittingMode
    ...
    case 'GWMCMC'
      ...
      logProbabilities = [logPrior, {likelihoodFun}]; 
      [models solution.logp, solution.result] = gwmcmc(...
        models0, logProbabilities, MAX_ITER, FITTINGARGS{:}...
      );
...
% params.
function [LL Lc]= loglikelihood1(params, RTs, conditions, modelParams, paramFn, N_MODEL_TRIALS)
  % LL = log likelihood of data given model paramters
  % Lc = log likelihood of each condition
  global iter 
  ...
  for(c=1:length(ucond))
    ...
    else  % Just one response:
     
      if(sum( t(:,1)<L )>5)
        p1=ksdensity( t( t(:,1)<L ,1 ), data( data(:,1)<L,1 ), 'function', 'pdf');
      else
        p1=0;
      end
      p1=p1* mean( t(:,1)<L );  % scale P(T1|R1) to P(T1).P(R1)

      Lc(c) = nansum(log( p1 + eps ));
      
    end % end if one or two responses.
  ...
  function plotParamsFunction(  RTs, conditionVector,...
    ...
    for(i=1:ncond) % for each condition
      if size(conditionVector,2)>1
        p(i)=varyParametersFunction( baseRaceParameters(i),p1, conditionVector(conditionVector,2:end) );
      else
