...
% If the first two inputs are different sizes, use ttest2 instead!

if nargin>1 && numel(varargin{1}) ~= numel(varargin{2})
  % two samples
  [h,p,ci,stat] = ttest2(varargin{:});
  fprintf('%0.3g (%0.3g) - %0.3g (%0.3g) = %0.3g;\t t(%g)=%0.3g, p=%0.3g\n', ...
    nanmean(varargin{1}), nanstd(varargin{1}), nanmean(varargin{2}), nanstd(varargin{2}), ...
    nanmean(varargin{1})-nanmean(varargin{2}),  ...
